...
Each normalized candidate is snapshotted as canonical JSON and keyed by SHA-256 together with its target game, making rediscovery idempotent while preserving provider, URL, language, document type, confidence, edition/version clues and metadata. Queue insertion resolves the target game before the write transaction and uses an immediate SQLite write transaction plus `INSERT ... ON CONFLICT(board_game_id, candidate_key) DO NOTHING`; concurrent rediscovery therefore converges on one row, while unrelated integrity failures remain visible. Database CHECK constraints encode the allowed relationships between policy action, status, decision source and decision timestamp rather than relying only on application code.

Persisted snapshots are revalidated against the P5A `RulebookCandidate` contract, canonical digest, denormalized columns and policy reasons before being returned or decided. Both persisted JSON fields are byte-bounded before decoding, and malformed, pathologically nested or otherwise unsafe JSON is converted into a quarantined corrupt-record result instead of escaping as a decoder/runtime failure. Corrupt rows are excluded from list output with an explicit corruption marker and cannot be approved through the decision API. Review transitions are atomic: `pending` may become `approved` or `rejected`; repeating the same decision is idempotent, while conflicting later decisions return a conflict instead of silently rewriting history. The web UI at `/reviews` consumes only the list/decision API, paginates pending and decided queues independently, and refreshes server state after a concurrent-decision conflict. P6A does not schedule discovery, fetch approved items, or create/update `game_documents`; those execution concerns remain P6B.

## Scheduled rulebook updates (P6B)

P6B turns an approved P6A review item into a persistent update target. It does not perform publisher discovery or search for replacement URLs: the approved normalized candidate remains the authority for the URL, provider, source class, language, document type and trust state. A lightweight background worker periodically synchronizes approved review items into update targets and re-fetches only targets whose `next_check_at` is due.

Every execution reuses the P5B `RulebookFetcher`; scheduled operation therefore inherits DNS/SSRF validation, pinned numeric connections, TLS hostname verification, redirect validation, absolute network deadlines, PDF validation, content limits and content-addressed download persistence. A successful fetch is revalidated again when entering the document archive: the fetched path must remain inside `/data/manuals`, its size, PDF signature and SHA-256 must still match fetch provenance, and a new `game_documents` row is created only when that game has not already archived the same SHA-256. Scheduled archive writes use no-follow directory descriptors rooted at the configured manuals directory and re-check directory identity before publication and before the database commit; a pre-existing or swapped game-directory symlink therefore cannot redirect the PDF outside the manuals root. Existing manual or fetched document metadata is never silently overwritten.

Update targets retain the interval, next due time, last check/success, last archived document/hash, consecutive failure count and last failure. Successful checks schedule the next run at the normal interval; failures use bounded exponential backoff. Every scheduler attempt is auditable, including pre-fetch validation failures. Failed runs retain a fetched SHA-256 when the guarded fetch had already produced one, while `document_id` remains null until archival has actually committed. Audit JSON is byte-bounded and type-validated when read; malformed, deeply nested or oversized audit JSON is quarantined instead of being fully deserialized into the response.

Scheduler concurrency is protected with database leases plus monotonically increasing fencing generations. Due work is claimed inside `BEGIN IMMEDIATE`; every claim increments `lease_generation`. A heartbeat extends a live lease while fetch/archive is running, and both completion and the `game_documents` insert verify the exact owner+generation fence. A worker that loses its lease may finish local/network work, but it cannot commit a document or target completion after a newer generation has been claimed. For a newly created version, the game-document row, successful run audit and target completion are written inside the same fenced SQLite transaction; an audit failure therefore rolls the document row back as well. The worker intentionally claims one target immediately before each execution instead of leasing an entire batch up front. Corrupt post-claim review data is audited as a validation failure, the lease is released, and the same scheduler cycle continues with later targets.

New targets are due immediately. Human approval creates the target synchronously; policy-approved items created by internal provider orchestration are discovered by the worker or the update-list synchronization pass. The default schedule is 30 days, with a 60-second worker poll, six-hour initial retry backoff, seven-day retry cap, 15-minute lease and batch size five. These are deployment settings, not trust inputs, and can be overridden with `BGC_RULEBOOK_UPDATE_*` environment variables. Shutdown is graceful: no new polling is started and lifespan waits for an already-running bounded worker thread before returning. The UI at `/updates` exposes target state, pause/resume, common schedule intervals and explicit “check now” execution, and periodically reconciles displayed state with the server.
