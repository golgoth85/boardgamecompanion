...
Every execution reuses the P5B `RulebookFetcher`; scheduled operation therefore inherits DNS/SSRF validation, pinned numeric connections, TLS hostname verification, redirect validation, absolute network deadlines, PDF validation, content limits and content-addressed download persistence. A successful fetch is revalidated again when entering the document archive: the fetched path must remain inside `/data/manuals`, its size, PDF signature and SHA-256 must still match fetch provenance, and a new `game_documents` row is created only when that game has not already archived the same SHA-256. Existing manual or fetched document metadata is never silently overwritten. The update run still records provenance when the fetched bytes are already present.

Update targets retain the interval, next due time, last check/success, last archived document/hash, consecutive failure count and last failure. Successful checks schedule the next run at the normal interval; failures use bounded exponential backoff. Every attempt also creates an immutable `rulebook_update_runs` audit row with outcome, URL/final URL, HTTP status, byte count, redirect chain and safe HTTP metadata.

Scheduler concurrency is protected with database leases. Due work is claimed inside `BEGIN IMMEDIATE`; a target can be reclaimed only after its lease expires. The worker intentionally claims one target immediately before each execution instead of leasing an entire batch up front, so later items cannot have their leases expire while earlier network operations consume the fetch timeout. Completion verifies lease ownership before writing the run/target state. Multiple application processes can therefore poll the same SQLite database without intentionally executing the same live lease twice.

New targets are due immediately. Human approval creates the target synchronously; policy-approved items created by internal provider orchestration are discovered by the worker or the update-list synchronization pass. The default schedule is 30 days, with a 60-second worker poll, six-hour initial retry backoff, seven-day retry cap, 15-minute lease and batch size five. These are deployment settings, not trust inputs, and can be overridden with `BGC_RULEBOOK_UPDATE_*` environment variables. The UI at `/updates` exposes target state, pause/resume, common schedule intervals and explicit “check now” execution.
